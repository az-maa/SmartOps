...
# nu ~ proportion d’anomalies attendues

y_pred = model.fit_predict(X_scaled)

# Ajouter la colonne anomalies (-1 = anomalie, 1 = normal)
df["anomaly_svm"] = y_pred
...
# Scores d'anomalie (distance à la frontière)
...
plt.scatter(df["cpu"], df["ram"], c=df["anomaly_svm"], cmap="coolwarm")
plt.xlabel("CPU")
plt.ylabel("RAM")
...
plt.figure(figsize=(8,6))
plt.scatter(df["disk"], df["network"], c=df["anomaly_svm"], cmap="coolwarm")
plt.xlabel("Disk")
plt.ylabel("Network")
plt.title("Anomalies détectées (Disk vs Network) - One-Class SVM")
plt.show()

plt.figure(figsize=(6,4))
df["anomaly_svm"].value_counts().plot(kind="bar", color=["red", "blue"])
plt.xticks([0,1], ["Normal (1)", "Anomalie (-1)"], rotation=0)
plt.title("Distribution des anomalies (One-Class SVM)")
